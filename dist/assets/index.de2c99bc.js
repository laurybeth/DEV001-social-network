...
const userState = (callback) => auth.onAuthStateChanged(callback);
...
const readCollectionUserDoc = (idDocUser) => {
  const docRef = sa(db, "users", idDocUser);
  return al(docRef);
};
const readAllCollectionPosts = (callback) => {
  const q2 = Mh(ea(db, "posts"), Lh("date", "asc"));
  return Il(q2, callback);
};
const deleteItem = (idDoc, collectionName) => {
...
>
  ...
  $registerForm.addEventListener("submit", (e) => {
    ...
    registerTasks(userName, userEmail, userBirthday, userPassword).then((docSnap) => {
  ...
  $linkGoogle.addEventListener("click", (e) => {
    e.preventDefault();
    registerGoogleTasks().then((docSnap) => {
      Modal("Welcome: ", `${docSnap.data().name}`);
      setTimeout(() => {
        document.getElementById("modal").style.display = "none";
      }, 2e3);
